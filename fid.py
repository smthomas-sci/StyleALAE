...
Script taken from original - https://github.com/bioinf-jku/TTUR

NOTE - THIS FILE HAS BEEN MODIFIED FROM THE ABOVE ORIGINAL

Modifications:
 - Modified to work with Tensorflow 2.+
 - Defaults to the model in the weights directory (doesn't download them)
 - Changed arguments to make explicit the real and fake image sets
 - If real statistics are not given, it will them for a future use by default.
 - Uses GPU every time.
...
def get_activations(images, sess, batch_size=12, verbose=True):
...
def calculate_activation_statistics(images, sess, batch_size=12, verbose=True):
...
def get_activations_from_files(files, sess, batch_size=24, verbose=True):
    """Calculates the activations of the pool_3 layer for all images.
...
def calculate_fid_given_paths(real_path, fake_path, inception_path, low_profile=False):
    ...
    with tf.Session() as sess:
        ...
        m1, s1 = _handle_path(real_path, sess, low_profile=low_profile)
        print("Processing fake images...")
        m2, s2 = _handle_path(fake_path, sess, low_profile=low_profile)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--real", type=str,
                        help='Path to the real images or to .npz statistic file')
    parser.add_argument("--fake", type=str,
                        help='Path to the fake images')
    args = parser.parse_args()
    # --------------------------------------------------------------------------------------- #

    # Set GPU use by default
    os.environ['CUDA_VISIBLE_DEVICES'] = "0"

    # Set weight path
    inception_path = os.path.join(os.path.dirname(fid.__file__), "weights/inception_v3/classify_image_graph_def.pb")
