...
with strategy.scope():
    ...
    # LOAD WEIGHTS
    OLD_WEIGHT_DIR = os.path.join(WEIGHT_DIR, "128x128_fine/")
    # Load weights if pre-trained
    F.load_weights(os.path.join(OLD_WEIGHT_DIR, f"F_{X_DIM}x{X_DIM}_weights.h5"))
    G.load_weights(os.path.join(OLD_WEIGHT_DIR, f"G_{X_DIM}x{X_DIM}_weights.h5"))
    E.load_weights(os.path.join(OLD_WEIGHT_DIR, f"E_{X_DIM}x{X_DIM}_weights.h5"))
    D.load_weights(os.path.join(OLD_WEIGHT_DIR, f"D_{X_DIM}x{X_DIM}_weights.h5"))

    # Build ALAE with submodels
...
callbacks = [Summary(log_dir=os.path.join(LOG_DIR, f"{X_DIM}x{X_DIM}/{m}"),
             ...
             ExponentialMovingAverage(weight_dir=WEIGHT_DIR, fid_dir=FID_DIR, save_images=True, k=10_000)
             ]
...
#
#         # LOAD WEIGHTS
#         OLD_WEIGHT_DIR = os.path.join(WEIGHT_DIR, "128x128_orig/")
#         DIM = 128
#         # Load weights if pre-trained
